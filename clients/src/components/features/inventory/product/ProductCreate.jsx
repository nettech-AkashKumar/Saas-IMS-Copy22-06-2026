...
const ProductCreate = () => {
  ...
  const fetchProduct = async () => {
    try {
      ...
      setFormData((prev) => ({
        ...
        itemBarcode: data.variants?.[0]?.itemBarcode || data.itemBarcode || "",
  ...
  const validateFinalSubmit = () => {
    ...
    if (settings.itembarcode) {
      // In edit mode, barcode lives in variant.barcode; in create mode also formData.itemBarcode
      const firstBarcode = String(variants[0]?.barcode || formData.itemBarcode || "").trim();
      if (!/^\d{13}$/.test(firstBarcode)) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (settings.itembarcode) {
      // prefer the barcode from the variant (what was actually generated/entered)
      const barcode = String(variants[0]?.barcode || formData.itemBarcode || "").trim();
      formPayload.append("itemBarcode", barcode);
    }
