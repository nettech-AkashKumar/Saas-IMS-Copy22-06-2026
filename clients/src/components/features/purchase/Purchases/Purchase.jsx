...
export default function Purchase() {
  ...
  const fetchPurchaseOrders = async (page = 1, limitOverride) => {
    try {
      setLoading(true);
  ...
  const fetchStats = async () => {
    try {
      ...
      setInitialStats(statsTop);
    }
  };

  // ✅ Handle menu actions with reorder support
  const handleMenuAction = (purchase, action) => {
    setSelectedInvoice(purchase);

    switch (action) {
      case "view":
        navigate(`/show-purchase/${purchase._id}`);
        break;
      case "print":
      ...
      case "debit_note":
        ...
        break;
      case "reorder":
        // ✅ Navigate to create purchase with the existing data for reordering
  ...
  const handleDeleteInvoice = async () => {
    ...
    try {
      const response = await api.delete(`/api/purchase/${selectedInvoice._id}`);
      if (response.data.success) {
  ...
  const handleExportExcel = () => {
    try {
      ...
      let ordersToExport = purchaseOrders.filter((order) =>
      ...
      const excelData = ordersToExport.map((order, index) => {
        ...
        const totalQty = getTotalQty(order.items);
      ...
      const filename = `selected-purchase-orders-${ordersToExport.length}-${format(new Date(), "yyyy-MM-dd")}`;
      XLSX.writeFile(wb, `${filename}.xlsx`);

      toast.success(`${ordersToExport.length} purchase order${ordersToExport.length > 1 ? 's' : ''} exported to Excel`);
  ...
  const handleRecordPayment = async (paymentData) => {
    try {
      const response = await api.post(
        `/api/purchase/${selectedOrderForPayment._id}/payment`,
        paymentData
      );
  ...
  const handlePrintPurchaseOrder = async (order) => {
    try {
      ...
      const response = await api.get(`/api/purchase/${order._id}`);
      if (response.data.success) {
        const purchaseOrderData = response.data.purchase;
                ...
                <head>
                  ...
                  <style>
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    body { font-family: 'IBM Plex Mono', 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; padding: 20px; background: white; }
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div className="d-flex align-items-center gap-3">
          ...
          {hasPermission(user, "Purchase", "create") && (
            <Link style={{ textDecoration: "none" }} to="/create-purchase">
              <button
        ...
        >
          <div className="d-flex align-items-center gap-4" style={{
            ...
            >
              ...
              <input
                type="search"
                placeholder="Search by supplier name, purchase no."
                value={search}
                      ...
                      >
                        ...
                        {openMenu === idx && (
                          <div
                            style={{
                              position: "fixed",
                              top: openUpwards ? dropdownPos.y - 150 : dropdownPos.y,
...
//     bg: "#F7F7C7",
//     dot: true,
//     label: "Pending",  // Changed from "Convert to Purchase"
//   },
//   received: {
//     color: "#059669",
//     dot: false,
...
//   },
//   {
//     label: "Record Payment",  // NEW MENU ITEM
//     icon: <FaMoneyBillWave size={18} />,
//     action: "payment",
//     permission: "update"
//   },
...
//   // If order is approved (received) - only show view, print, payment
//   if (order.status === "received") {
//     return menuItems.filter(item =>
//       item.action === "view" ||
//       item.action === "print" ||
//       item.action === "payment" ||
//       item.action === "debit_note"
//     );
//   }

//   // If order is cancelled - only show view and print
...
//     );
//   }
//   // For pending (converted) orders - show all actions
//   return menuItems.filter(item =>
//     item.action !== "debit_note"  // ✅ Remove debit note for pending orders
//   );
// };
// export default function Purchase() {
//   const { user } = useAuth();
...
//   //       setTotalCount(response.data.total || 0);
//   //       setTotalPages(response.data.pagination?.totalPages || 1);
//   //       if (page === 1 && !status && !search && !selectedDateRange.startDate && !selectedDateRange.endDate) {
//   //         // Update tab counts
//   //         updateTabCounts(response.data.purchases || []);
//   //          updateStats(response.data.purchases || []);
//   //       }
...
//     try {
//       setLoading(true);
//       // Use limitOverride if provided, otherwise use itemsPerPage state
//       const currentLimit = limitOverride !== undefined ? limitOverride : itemsPerPage;
...
//         // Update itemsPerPage from response if needed
//         if (response.data.pagination?.limit && limitOverride === undefined) {
//           setItemsPerPage(response.data.pagination.limit);
//         }
...
//         //   updateStats(response.data.purchases || []);
//         // }
//         // ALWAYS fetch fresh counts for tabs, regardless of page/filters
//         await fetchTabCounts();

//         // ALWAYS fetch fresh stats regardless of page/filters
//         await fetchStats();
//       } else {
//         toast.error(response.data.error || "Failed to load purchase orders");
//       }
...
//     try {
//       const response = await api.get("/api/purchase/status-counts"); // already correct
//       if (response.data.success) {
//         const counts = response.data.counts;
//         // console.log('couuntssss', counts)
//         setInitialTabs([
//           { ...tabsData[0], count: counts.all },
//           { ...tabsData[1], count: counts.converted },
//           { ...tabsData[2], count: counts.received },
...
//     try {
//       const response = await api.post(
//         `/api/purchase/${purchase._id}/duplicate`
//       );
//       if (response.data.success) {
...
//         `/api/purchase/${selectedInvoice._id}`
//       );
//       if (response.data.success) {
//         toast.success("Purchase order deleted successfully");
...
//         status = "";
//     }
//     // console.log("Page change - activeTab:", activeTab, "status:", status, "page:", page);
...
//       await api.put(`/api/purchase/${selectedInvoice._id}`, updateData);
...
//         if (order.status === "converted") statusDisplay = "Pending";
...
//         // ✅ FIXED: Use correct prop name "order" instead of "purchaseOrder"
//         root.render(
//           <PurchaseOrderContent
//             order={purchaseOrderData}  // Changed from purchaseOrder to order
//             supplier={purchaseOrderData.supplierId}
//             companyData={companyData}
//             banks={banks}
//             terms={terms}
...
//                       {/* total qty */}
//                       <td style={{ fontFamily: "inter 'sans-serif", fontWeight: 400, fontSize: "14px", lineHeight: "120%", padding: "6px 16px", color: "#0E101A" }}>{ }</td>
...
//                           {/* <span>{getArrivingDate(order.purchaseDate)}</span> */}
//                         </div>
//                       </td>
