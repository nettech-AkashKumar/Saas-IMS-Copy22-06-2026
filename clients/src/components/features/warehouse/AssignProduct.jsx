...
function AssignProduct() {
    ...
    try {
      ...
      setProducts(
        (res.data.products || []).map((product) => {
          ...
          const dbAssigned = Number(product.assignedQuantity || 0); // ✅ backend keeps this updated
  ...
  // Core drill-down logic:
  // Given sv (selectedValues) and sl (selectedLocation) with warehouse already set,
  // auto-select singles at each level and only add a field to visibleFields when count > 1.
  // Drilling stops at any level where the user still needs to pick (count > 1 and nothing chosen).
  const drillDown = (sv, sl, warehouseObj) => {
    ...
    if (zonesData.length === 1) {
      ...
      if (!resolvedZone) {
        // User hasn't picked yet — stop here
        setRacks([]);
        setShelves([]);
        setBins([]);
        setSelectedValues(sv);
    ...
    if (binsData.length === 1) {
      ...
      sl = { ...sl, bin: autoBin };
      sv = { ...sv, bin: typeof autoBin === "string" ? autoBin : autoBin.binName };
    } else {
      newVisible.push("bin");
    }
  ...
  const handleAssignProduct = async () => {
    try {
      ...
      const invalidProduct = selectedProducts.find((p) => !p.assignQty || Number(p.assignQty) < 1);
      if (invalidProduct) {
        toast.error(`Please enter a quantity of at least 1 for "${invalidProduct.productName}"`);
        return;
      }
  ...
  const handleDropdownSelect = (field, item) => {
    ...
    if (field === "warehouse") {
      // Full reset downstream, then drill from scratch
    ...
    if (field === "zone") {
      // Reset everything below zone, then re-drill with this zone chosen
    ...
    if (field === "rack") {
      // Reset everything below rack, then re-drill with this rack chosen
    ...
    if (field === "shelf") {
      // Reset everything below shelf, then re-drill with this shelf chosen
        ...
        >
          <form style={{
            ...
            <div style={{ borderBottom: "1px solid #EAEAEA" }}>
              <div>
                <h3 style={{ fontSize: "16px", fontWeight: 500, marginBottom: "10px", color: "#000000" }}>
                  Product Details
                </h3>
              </div>
              ...
              <div ref={searchRef} style={{ position: "relative", marginBottom: "10px" }}>
                <FiSearch
                  size={18}
                  color="#9CA3AF"
                  style={{ position: "absolute", top: "50%", left: "14px", transform: "translateY(-50%)", zIndex: 1 }}
                />
                <input
                  type="text"
                      ...
                      >
                        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: "12px", color: "#727681", marginTop: "2px" }}>
                              {item.itemBarcode}
                            </div>
                          </div>
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            const alreadyAdded = selectedProducts.some((p) => p._id === item._id);
                            if (alreadyAdded) {
                              setSelectedProducts((prev) => prev.filter((p) => p._id !== item._id));
                            } else {
                              setSelectedProducts((prev) => [
                                ...prev,
                                { ...item, assignQty: 0, unit: item.unit || "Pcs" },
                              ]);
                            }
                          }}
                          style={{
                            background: selectedProducts.some((p) => p._id === item._id) ? "#ffffff" : "#1F7FFF",
                            color: selectedProducts.some((p) => p._id === item._id) ? "#1F7FFF" : "#fff",
                        ...
                        >
                          {selectedProducts.some((p) => p._id === item._id) ? "Added" : "+ Add"}
                        </button>
                      </div>
                    ))}
                  </div>
              ...
              {/* TABLE */}
              <div style={{ overflowY: "auto", width: "100%", height: "330px" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ background: "#F3F8FB", height: "54px" }}>
                      {["Sr No.", "Product Name", "Assigned Qty", "Unit", "Available Qty", "Action"].map((h, i) => (
                        <th
                          key={i}
                          style={{
                  ...
                  <tbody>
                    {selectedProducts.length === 0 ? (
                      <tr>
                        <td colSpan="6">
                          <div style={{ height: "250px", display: "flex", justifyContent: "center", alignItems: "center", fontSize: "16px", fontWeight: 500, color: "#727681" }}>
                            No Product Selected
                          </div>
                        </td>
                      </tr>
                      ...
                      selectedProducts.map((item, index) => (
                        <tr key={item._id} style={{ borderBottom: "1px solid #EAEAEA" }}>
                          <td style={{ padding: "12px 28px", fontSize: 14, color: "#0E101A" }}>{index + 1}</td>
                          <td style={{ padding: "12px 28px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                              <img
                                src={item.images?.[0]?.url || ProductDefaultImage}
                                alt="product"
                                style={{ width: "44px", height: "44px", borderRadius: "6px", objectFit: "cover", border: "1px solid #F1F1F1" }}
                              />
                              <div style={{ fontSize: "14px", color: "#0E101A", fontWeight: 500 }}>
                                {item.productName}
                              </div>
                            </div>
                          </td>
                          <td style={{ padding: "12px 28px" }}>
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                const totalStock = Number(dbProduct?.stockQuantity || 0);
                                const totalAllocated = allocatedMap[item._id?.toString()] || 0;
                                ...
                                if (value > maxQty) {
                                  toast.error(`Only ${maxQty} quantity available`);
                                  value = maxQty;
                                }
                                const updated = [...selectedProducts];
                              ...
                              style={{ width: "120px", height: "40px", border: "1px solid #D1D5DB", borderRadius: "8px", padding: "0 12px", fontSize: "14px", outline: "none" }}
                            />
                          </td>
                          <td style={{ padding: "12px 28px", fontSize: "14px", color: "#0E101A" }}>
                            {item.unit || "Pcs"}
                          </td>
                          <td style={{ padding: "12px 28px" }}>
                            <div style={{ width: "140px", height: "40px", border: "1px solid #D1D5DB", borderRadius: "8px", padding: "0 12px", display: "flex", alignItems: "center", background: "#fff" }}>
                              <span>
                                {(() => {
                                  const dbProduct = products.find((p) => p._id === item._id);
                                  const totalStock = Number(dbProduct?.stockQuantity || 0);
                                  const totalAllocated = allocatedMap[item._id?.toString()] || 0;
                                  return totalStock - totalAllocated - Number(item.assignQty || 0);
                                })()}
                              </span>
                            </div>
                          </td>
                          <td style={{ padding: "12px 28px", textAlign: "center" }}>
                            <FiTrash2
                              size={18}
                              color="#FF4D4F"
            ...
            {/* Location Section */}
            <div>
              <span style={{ margin: 0, color: "#000000", fontSize: "16px", fontWeight: 500 }}>
                Assign to Location
              </span>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "20px", width: "100%" }}>
                  {visibleFields.map((field) => (
                    <div key={field} style={{ minWidth: "200px", flex: "1" }}>
                      <label style={{ fontSize: "12px", color: "#727681", marginBottom: "6px", display: "block", textTransform: "capitalize" }}>
                        {field}
                      </label>
                      <div
                      ...
                      >
                        <div
                          onClick={() => setDropdowns((prev) => ({ ...prev, [field]: !prev[field] }))}
                          style={{ width: "100%", fontSize: "14px", color: selectedValues[field] ? "#0E101A" : "#727681" }}
                        >
                          {selectedValues[field] || `Select ${field}`}
                        </div>
                        <div onClick={() => setDropdowns((prev) => ({ ...prev, [field]: !prev[field] }))}>
                          {dropdowns[field] ? <IoIosArrowUp /> : <IoIosArrowDown />}
                        </div>
                        ...
                        {dropdowns[field] && (
                          ...
                          >
                            {dropdownOptions[field].length === 0 ? (
                              <div style={{ padding: "10px 14px", fontSize: "14px", color: "#727681" }}>
                              ...
                              dropdownOptions[field].map((item, index) => (
                                <div
                                  key={index}
                                  onClick={() => handleDropdownSelect(field, item)}
                                  className="button-hover"
                                  style={{
                                    padding: "10px 14px",
                                    fontSize: "14px",
                                    cursor: "pointer",
                                    borderBottom: index !== dropdownOptions[field].length - 1 ? "1px solid #F3F4F6" : "none",
                                  }}
                                >
                                  {field === "warehouse" ? item.warehouseName
                                    : field === "zone" ? item.zoneName
                                      : field === "rack" ? item.rackName
                                        : field === "shelf" ? item.shelfName
                                          : field === "bin" ? item.binName
                                            : ""}
                                </div>
                              ))
                            )}
