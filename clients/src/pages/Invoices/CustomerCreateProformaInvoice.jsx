...
// import PreviewProformaInvoice from "./PreviewProformaInvoice";
...
function CustomerCreateProformaInvoice() {
  const COL = {
    sl: 80,
    item: 260,
    md: 120,
  ...
  const viewValidUntilRef = useRef(null);
  const hasAddedInitialProduct = useRef(false);
  const { proformaId, quotationId, customerId } = useParams();
  const navigate = useNavigate();
  const params = useParams();
  // console.log("=== ALL URL PARAMS ===");
  // console.log("params object:", params);
  // console.log("params.id:", params.id);
  // console.log("params.proformaId:", params.proformaId);

  // console.log("Extracted proformaId:", proformaId);
  ...
  // Add this to get location state for quotation conversion
  ...
  const viewProformaData = location.state?.viewProforma;
  const editProformaData = location.state?.editProforma;
  const mode = location.state?.mode;
  // Determine modes
  const isViewMode = mode === 'view';
  const isEditModeProforma = mode === 'edit';
  const hasExistingProformaData = viewProformaData || editProformaData;
  // ✅ Determine if we're in edit mode (either from URL param OR from state)
  const isEditMode = !!proformaId || !!editProformaData?._id;
  // ✅ Get the actual ID to use for API calls
  const existingProformaId = proformaId || editProformaData?._id;

  const [isReadOnly, setIsReadOnly] = useState(isViewMode);
  // console.log("isReadOnly:", isReadOnly);
  // console.log("isEditMode:", isEditMode);
  // console.log("isViewMode:", isViewMode);
  // console.log("editProformaData:", editProformaData);
  // Add these state variables
  const [dueAmount, setDueAmount] = useState(0);
  const [advancePaymentStatus, setAdvancePaymentStatus] = useState("pending");
  // for barcode scanner input
  const productSearchInputRef = useRef(null);

  // Add this state to track if we're converting from quotation
  ...
  const [isValidUntilPickerOpen, setIsValidUntilPickerOpen] = useState(false);
  ...
  const [viewProformaOptions, setViewProformaOptions] = useState(false);
  const [viewValidUntilOptions, setViewValidUntilOptions] = useState(false);
  ...
  // Proforma specific dates
  const [proformaDate, setProformaDate] = useState(new Date());
  const [validUntil, setValidUntil] = useState(() => addDays(new Date(), 30));
  const [proformaNo, setProformaNo] = useState("");
  const [quotationIdRef, setQuotationIdRef] = useState(quotationId || null);
  ...
  useEffect(() => {
    // console.log("=== Fetch Proforma Debug ===");
    // console.log("existingProformaId:", existingProformaId);
    // console.log("isEditMode:", isEditMode);
    // console.log("hasAddedInitialProduct.current:", hasAddedInitialProduct.current);

    const fetchProformaForEdit = async () => {
      if (existingProformaId && !hasAddedInitialProduct.current) {
        // console.log("Fetching proforma with ID:", existingProformaId);
        try {
          const response = await api.get(`/api/proforma-invoices/${existingProformaId}`);
          // console.log("API Response:", response.data);

          if (response.data.success) {
            const proforma = response.data.data;
            // console.log("Proforma data received:", proforma);

            // Populate form with proforma data
            populateFormWithProformaData(proforma);
            hasAddedInitialProduct.current = true;
            // toast.success("Proforma loaded successfully");
          }
        } catch (error) {
        ...
        console.log("No proformaId found, this is create mode");
      }
    };

    fetchProformaForEdit();
  }, [existingProformaId]);
  ...
  //   };
  //   if (!isEditMode && !proformaNo) {
  //     setProformaNo(generateProformaNumber());
  //   }
  // }, [isEditMode]);
  ...
  const closeAllDropdowns = () => {
    ...
    setViewValidUntilOptions(false);
    setIsValidUntilPickerOpen(false);
  };

  const handleViewManage = () => { setViewValidUntilOptions(false); setIsValidUntilPickerOpen(false); setViewManageOptions(prev => !prev); };
  const handleViewProforma = (open) => setViewProformaOptions(open);
  ...
  // Helper function to populate form with proforma data (similar to quotation)
  const populateFormWithProformaData = (proforma) => {
    // Set customer data
    if (proforma.customerId) {
      const customerData = proforma.customerId;
      const addressParts = [];
  ...
  const attachments = proforma.attachments.map(att => ({
    ...
    isExisting: true  // ← Add this flag to identify existing attachments
  }));
  setUploadedImages(attachments);
}

    // Set proforma basic info
    if (proforma.proformaDate) setProformaDate(new Date(proforma.proformaDate));
    if (proforma.validUntil) setValidUntil(new Date(proforma.validUntil));
    if (proforma.proformaNo) setProformaNo(proforma.proformaNo);

    // Set billing address if available
    if (proforma.billingAddress) {
      setCustomer(prev => ({ ...prev, address: proforma.billingAddress }));
    }

    // Set payment terms
    if (proforma.paymentTerms) setPaymentTerms(proforma.paymentTerms);
    if (proforma.advanceAmount) setAdvanceAmount(proforma.advanceAmount);
    if (proforma.advancePaid) setAdvancePaid(proforma.advancePaid);

    // Set products
    if (proforma.items && proforma.items.length > 0) {
      const loadedProducts = proforma.items.map((item, index) => ({
    ...
    if (proforma.additionalDiscount) {
      if (proforma.additionalDiscount.pct > 0) {
        setAdditionalDiscountType("Percentage");
        setAdditionalDiscountPct(proforma.additionalDiscount.pct);
      } else if (proforma.additionalDiscount.amt > 0) {
        setAdditionalDiscountType("Fixed");
        setAdditionalDiscountAmt(proforma.additionalDiscount.amt);
      }
    }

    // Set additional charges
    if (proforma.additionalChargesDetails) {
      setAdditionalChargesDetails({
        shipping: proforma.additionalChargesDetails.shipping || 0,
        handling: proforma.additionalChargesDetails.handling || 0,
        packing: proforma.additionalChargesDetails.packing || 0,
        service: proforma.additionalChargesDetails.service || 0,
        other: proforma.additionalChargesDetails.other || 0,
      });
    }

    // Set tax settings
    if (proforma.taxSettings) {
      setTaxSettings(proforma.taxSettings);
    }
  };
  // Load proforma data for view/edit from state (similar to quotation)
  useEffect(() => {
    const loadProformaData = () => {
      if (viewProformaData) {
        populateFormWithProformaData(viewProformaData);
        setIsReadOnly(true);
        toast.info("Viewing proforma invoice");
      } else if (editProformaData) {
        populateFormWithProformaData(editProformaData);
        setIsReadOnly(false);
        toast.info("Editing proforma invoice - You can modify and save changes");
      }
    };

    if (hasExistingProformaData && !hasAddedInitialProduct.current) {
      loadProformaData();
    }
  }, [viewProformaData, editProformaData]);
  ...
  // const isFromNavbar = !proformaId && !customerId && !editProformaData?._id;
  ...
  const handleValidUntilSelect = (option) => {
    const today = new Date();
    let selectedDate = new Date();
    ...
    switch (option) {
      ...
      case "Custom":
        setViewValidUntilOptions(false);
        setIsValidUntilPickerOpen(true);
        break;
    }
  };
  ...
  const handleSearch = (e, rowId) => {
    ...
    setSearchData((prev) => {
      ...
      Object.keys(prev).forEach((id) => {
        updated[id] = {
          ...prev[id],
          isOpen: id === String(rowId),
          term: id === String(rowId) ? term : prev[id].term,
          filtered: id === String(rowId) ? filtered : prev[id].filtered,
        };
      });
      return updated;
    });
  ...
  // Add this useEffect to close Valid Until dropdown when clicking outside the date picker
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Check if clicking outside the date picker when it's open
      if (isValidUntilPickerOpen) {
        const datePickerElement = document.querySelector('.valid-until-datepicker');
  ...
  // Sync amountReceived with advancePaid
  useEffect(() => {
    const receivedAmount = parseFloat(amountReceived) || 0;
    setAdvancePaid(receivedAmount);
  }, [amountReceived]);
  ...
  useEffect(() => {
    // Calculate due amount whenever grandTotal or advancePaid changes
    const calculatedDue = grandTotal - (parseFloat(advancePaid) || 0);
    setDueAmount(calculatedDue);
  }, [grandTotal, advancePaid]);

  // Also calculate advance payment status
  useEffect(() => {
    ...
    setAdvancePaymentStatus(status);
  }, [advanceAmount, advancePaid]);

  // Load quotation data when converting from quotation
  useEffect(() => {
    const loadQuotationForConversion = async () => {
      if (sourceQuotation && isFromQuotation) {
        ...
        // Convert quotation items to proforma items format
        ...
        toast.info("Quotation loaded. You can modify terms before creating Proforma Invoice.", {
          position: "top-right"
        });
      }
    };
  ...
  const handleDateSelect = (option, e) => {
  ...
  // Handle form submission for Proforma Invoice
  ...
  //   const nonEmptyProducts = products.filter(p => p.productId && p.productId.trim() !== "" && p.itemName && p.itemName.trim() !== "");
  //   if (nonEmptyProducts.length === 0) {
  //     toast.error("Please add at least one product");
  //     return;
  //   }
  //   if (!validateProductsBeforeSave()) return;

  //   setIsSubmitting(true);

  //   try {
  //     // CRITICAL FIX: Get the advance amount and paid amount values
  //     const advanceAmountValue = parseFloat(advanceAmount) || 0;
  //     const advancePaidValue = parseFloat(advancePaid) || 0;
  //     const grandTotalValue = parseFloat(grandTotal) || 0;

  //     // Calculate due amount
  //     const calculatedDueAmount = Math.max(0, grandTotalValue - advancePaidValue);

  //     // Calculate payment status
  //     let paymentStatus = "pending";
  //     if (advanceAmountValue > 0) {
  //       if (advancePaidValue >= advanceAmountValue) {
  //         paymentStatus = "paid";
  //       } else if (advancePaidValue > 0) {
  ...
  //     // });
  //     // Send as JSON instead of FormData (temporarily remove file upload)
  //     const payload = {
  ...
  //       additionalDiscount: {
  //         pct: parseFloat(additionalDiscountPct) || 0,
  //         amt: parseFloat(additionalDiscountAmt) || 0
  //       },
  //       additionalChargesDetails: {
  //         shipping: additionalChargesDetails.shipping,
  //         handling: additionalChargesDetails.handling,
  //         packing: additionalChargesDetails.packing,
  //         service: additionalChargesDetails.service,
  //         other: additionalChargesDetails.other,
  //       },
  //       taxSettings: {
  //         enableGSTBilling: taxSettings.enableGSTBilling,
  //         priceIncludeGST: taxSettings.priceIncludeGST,
  //         autoRoundOff: taxSettings.autoRoundOff,
  ...
  //         taxType: `GST ${p.taxRate}%`,
  //         taxRate: p.taxRate,
  //         taxAmount: p.taxAmount,
  //         discountPct: parseFloat(p.discountPct) || 0,
  ...
  //     // Add quotationId if converting from quotation
  //     if (quotationIdRef) {
  //       payload.quotationId = quotationIdRef;
  //     }

  //     // console.log("Sending payload with payment:", {
  //     //   advanceAmount: payload.advanceAmount,
  //     //   advancePaid: payload.advancePaid,
  //     //   dueAmount: payload.dueAmount,
  //     //   advancePaymentStatus: payload.advancePaymentStatus
  //     // });

  //     let response;
  //     if (existingProformaId) {
  //       // console.log("Updating proforma with ID:", existingProformaId);
  //       response = await api.put(`/api/proforma-invoices/${existingProformaId}`, payload);
  //       toast.success("Proforma Invoice updated successfully!");
  //     } else {
  //       // console.log("Creating new proforma");
  //       response = await api.post("/api/proforma-invoices", payload);
  //       toast.success("Proforma Invoice created successfully!");

  //       if (response.data.success && quotationIdRef) {
  //         try {
  //           await api.put(`/api/quotations/${quotationIdRef}/status`, {
  //             status: "converted_to_proforma"
  //           });
  //         } catch (statusErr) {
  //           console.log("Could not update quotation status:", statusErr.message);
  //         }
  //       }
  //     }

  //     if (response.data.success) {
  //       const newProformaId = response.data.proforma._id;
  //       if (shouldPrint) {
  //         navigate(`/skeleton?redirect=/showproforma/${newProformaId}`);
  //       } else {
  //         navigate(`/showproforma/${newProformaId}`);
  //       }
  //     }
  //   } catch (error) {
  //     console.error("Error:", error);
  //     toast.error(error?.response?.data?.message || error?.message || "Proforma Invoice operation failed");
  //   } finally {
  //     setIsSubmitting(false);
  //   }
  // };
// Handle form submission for Proforma Invoice with attachments
const handleSubmit = async (shouldPrint = false) => {
  if (!customer.customerId) {
    toast.error("Please select a customer first");
    return;
  ...
  try {
    ...
    // Add all proforma data as fields
    formData.append("customerId", customer.customerId);
    formData.append("proformaDate", proformaDate.toISOString());
    ...
    formData.append("advanceAmount", parseFloat(advanceAmount) || 0);
    formData.append("advancePaid", parseFloat(advancePaid) || 0);
    formData.append("dueAmount", Math.max(0, grandTotal - (parseFloat(advancePaid) || 0)));
    ...
    let paymentStatus = "pending";
    if (advanceAmt > 0) {
      if (paidAmt >= advanceAmt) paymentStatus = "paid";
      else if (paidAmt > 0) paymentStatus = "partial";
    }
    formData.append("advancePaymentStatus", paymentStatus);
    ...
    formData.append("additionalChargesDetails[shipping]", additionalChargesDetails.shipping);
    formData.append("additionalChargesDetails[handling]", additionalChargesDetails.handling);
    formData.append("additionalChargesDetails[packing]", additionalChargesDetails.packing);
    formData.append("additionalChargesDetails[service]", additionalChargesDetails.service);
    formData.append("additionalChargesDetails[other]", additionalChargesDetails.other);

    // Add tax settings
    formData.append("taxSettings[enableGSTBilling]", taxSettings.enableGSTBilling);
    formData.append("taxSettings[priceIncludeGST]", taxSettings.priceIncludeGST);
    formData.append("taxSettings[autoRoundOff]", taxSettings.autoRoundOff);
    formData.append("taxSettings[defaultGSTRate]", taxSettings.defaultGSTRate);
    ...
    nonEmptyProducts.forEach((p, index) => {
      ...
      formData.append(`items[${index}][selectedSerialNos]`, (p.selectedSerialNos || []).join(","));
    });

    // Add quotationId if converting from quotation
    if (quotationIdRef) {
      formData.append("quotationId", quotationIdRef);
    }

    // ✅ FIX: Separate existing attachments from new files
    // Track which attachments to keep (existing ones)
...
// ✅ Always append — even empty array tells backend "delete everything"
formData.append("existingAttachments", JSON.stringify(existingAttachments));
    
    // Add only new files (actual File objects)
    ...
    let response;
    if (existingProformaId) {
      // Update existing proforma with FormData
      response = await api.put(`/api/proforma-invoices/${existingProformaId}`, formData, {
        headers: {
      ...
      // Create new proforma with FormData
      response = await api.post("/api/proforma-invoices", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      const clickedInsideInvoiceDate = viewManageRef.current?.contains(event.target);
      const clickedInsideValidUntil = viewValidUntilRef.current?.contains(event.target);

      if (!clickedInsideInvoiceDate) {
        setViewManageOptions(false);
        setIsDatePickerOpen(false);
      }
      if (!clickedInsideValidUntil) {
        setViewValidUntilOptions(false);
        setIsValidUntilPickerOpen(false);
      }
    };
        ...
        <div className="">
          ...
          >
            ...
            >
              ...
              >
                {isViewMode ? "View Proforma Invoice" : (isEditModeProforma ? "Edit Proforma Invoice" : (isConvertingFromQuotation ? "Convert Quotation to Proforma Invoice" : "Create Proforma Invoice"))}
              </h2>
            </div>
            ...
            >
              <div
                onClick={() => handleViewProforma(true)}
                style={{
                  padding: "6px 16px",
                  background: "#1F7FFF",
            ...
            <div style={{ width: "100%" }}>
              ...
              >
                ...
                >
                        ...
                        <div style={{ position: "relative", width: 200 }}>
                          ...
                          <div
                            ...
                            onClick={(e) => {
                              ...
                              setViewValidUntilOptions(false);
                              setIsValidUntilPickerOpen(false);
                              setViewManageOptions(prev => !prev);
                            }}
                          >
                            <LuCalendarMinus2 />
                            <div
                          ...
                          {/* DatePicker for Custom selection - INVOICE DATE */}
                          {isDatePickerOpen && (
                            <div
                              style={{
                                position: "absolute",
                        ...
                        <div style={{ position: "relative", width: 200 }}>
                          ...
                          <div
                            ...
                            onClick={(e) => {
                              ...
                              setViewManageOptions(false);
                              setIsDatePickerOpen(false);
                              setViewValidUntilOptions(prev => !prev);
                            }}
                          >
                            <LuCalendarMinus2 />
                            <div>{format(validUntil, "dd MMM yyyy")}</div>
                            <FiChevronDown />
                            {viewValidUntilOptions && (
                              <div
                                style={{
                                  position: "absolute",
                                  top: "35px",
                              ...
                              >
                                ...
                                >
                                  {["15 Days", "30 Days", "45 Days", "60 Days", "90 Days", "Custom"].map((option) => (
                                    <div
                                      key={option}
                                      style={{
                                        display: "flex",
                                      ...
                                      onMouseEnter={(e) => e.currentTarget.style.backgroundColor = "#f3f7ff"}
                                      onMouseLeave={(e) => e.currentTarget.style.backgroundColor = "transparent"}
                                      onClick={() => handleValidUntilSelect(option)}
                                    >
                                      <span style={{ color: "black" }}>{option}</span>
                                    </div>
                                  ))}
                  ...
                  {/* Valid Until DatePicker - OUTSIDE all divs */}
                  {isValidUntilPickerOpen && (
                    <div
                      className="valid-until-datepicker"
                      style={{
                        position: "fixed",
                        top: viewValidUntilRef.current ? viewValidUntilRef.current.getBoundingClientRect().bottom + window.scrollY + 5 : "50%",
                        left: viewValidUntilRef.current ? viewValidUntilRef.current.getBoundingClientRect().left + window.scrollX : "50%",
                        transform: viewValidUntilRef.current ? "none" : "translate(-50%, -50%)",
                        zIndex: 1000000,
                        background: "white",
                        padding: "10px",
                        borderRadius: "8px",
              ...
              >
                ...
                >
                  <div
                    ...
                    // onClick={() => setViewProformaOptions(true)}
                    onClick={() => {
                        ...
                        >
                          ...
                          >
                            ...
                            <div
                              ...
                              onClick={(e) => {
                                if (isReadOnly) e.stopPropagation();  // ← ADD THIS LINE
                              }}
                            >
                              <input
                                data-row-id={p.id}
...
<div>
  ...
  {/* Show upload button only in edit/create mode */}
  {!isReadOnly && (
    <div
      style={{
        width: "80px",
  ...
  {/* Display existing attachments (always show, even in read-only mode) */}
    ...
    >
      {uploadedImages.map((file, index) => (
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        <input
                          placeholder="0.00"
                          readOnly={isReadOnly || fullyReceived}
                          className=""
                          value={amountReceived}
                          onChange={(e) => {
                            const value = e.target.value;
                            setAmountReceived(value);
                            setAdvancePaid(parseFloat(value) || 0);  // ← ADD THIS LINE
                          }}
                          // bug done
                          onFocus={(e) => {
        ...
        <PreviewProformaInvoice
          isOpen={viewProformaOptions}
          onClose={() => setViewProformaOptions(false)}
          proformaData={{
            proformaNo,
            proformaDate,
            validUntil,
...
export default CustomerCreateProformaInvoice;
