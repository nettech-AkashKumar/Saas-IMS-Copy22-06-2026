...
import PreviewSalesOrder from "./PreviewSalesOrder";
...
function CustomerCreateSalesOrder() {
  const COL = {
    sl: 80,
    item: 260,
    md: 120,
  ...
  const [isReadOnly, setIsReadOnly] = useState(() => {
    // If URL has ID and mode is 'view', set read-only
    if (id && location.state?.mode === 'view') return true;
    if (id && !location.state?.mode) return true; // Default to view if just ID
    return false;
  })
  ...
  // Helper function to populate form with sales order data
  const populateFormWithSalesOrderData = (salesOrder) => {
    // Set customer data
    if (salesOrder.customerId) {
      const customerData = salesOrder.customerId;
      const addressParts = [];
    ...
    // Set sales order basic info
    if (salesOrder.orderDate) setInvoiceDate(new Date(salesOrder.orderDate));
    if (salesOrder.expectedDeliveryDate) setDueDate(new Date(salesOrder.expectedDeliveryDate));
    if (salesOrder.salesOrderNo) setInvoiceNo(salesOrder.salesOrderNo);
    if (salesOrder.status) setStatus(salesOrder.status);

    // Set billing address if available
    if (salesOrder.billingAddress) {
      setCustomer(prev => ({ ...prev, address: salesOrder.billingAddress }));
    }

    // Set advance payment info
    if (salesOrder.advanceAmount) setAdvanceAmount(salesOrder.advanceAmount);
    if (salesOrder.advancePaid) setAdvancePaid(salesOrder.advancePaid);
    if (salesOrder.dueAmount) setDueAmount(salesOrder.dueAmount);
    if (salesOrder.advancePaymentStatus) setAdvancePaymentStatus(salesOrder.advancePaymentStatus);

    // Set products
    if (salesOrder.items && salesOrder.items.length > 0) {
      const loadedProducts = salesOrder.items.map((item, index) => ({
      ...
      // Add empty product row if no products
    ...
    if (salesOrder.additionalDiscount) {
      if (salesOrder.additionalDiscount.pct > 0) {
        setAdditionalDiscountType("Percentage");
        setAdditionalDiscountPct(salesOrder.additionalDiscount.pct);
      } else if (salesOrder.additionalDiscount.amt > 0) {
        setAdditionalDiscountType("Fixed");
        setAdditionalDiscountAmt(salesOrder.additionalDiscount.amt);
      }
    }

    // Set additional charges
    if (salesOrder.additionalChargesDetails) {
      setAdditionalChargesDetails({
        shipping: salesOrder.additionalChargesDetails.shipping || 0,
        handling: salesOrder.additionalChargesDetails.handling || 0,
        packing: salesOrder.additionalChargesDetails.packing || 0,
        service: salesOrder.additionalChargesDetails.service || 0,
        other: salesOrder.additionalChargesDetails.other || 0,
      });
    }

    // Set tax settings
    if (salesOrder.taxSettings) {
      setTaxSettings(salesOrder.taxSettings);
    }

    // Set attachments if any
    if (salesOrder.attachments && salesOrder.attachments.length > 0) {
      const attachments = salesOrder.attachments.map(att => ({
        file: null,
        url: att.url,
        preview: att.url,
    ...
    // Set fully received status
    const advancePaidAmount = salesOrder.advancePaid || 0;
    const totalAmount = salesOrder.grandTotal || 0;
    if (advancePaidAmount >= totalAmount && totalAmount > 0) {
      setFullyReceived(true);
      setAmountReceived(advancePaidAmount.toString());
    } else if (advancePaidAmount > 0) {
  ...
  // Fetch sales order by ID for view/edit
  const fetchSalesOrderById = async (salesOrderId) => {
    try {
      ...
      if (response.data.success) {
        ...
        populateFormWithSalesOrderData(salesOrder);

        // Check if we're in view mode or edit mode
        // You can determine this by a query param or state
        const isViewOnly = location.state?.mode === 'view';
  ...
  const fetchCompanyData = async () => {
    try {
      ...
      toast.error(error?.res?.data?.displayMessage ||
    ...
    const loadProformaForConversion = async () => {
      if (sourceProforma && isFromProforma) {
        ...
        if (proformaAdvancePaid > 0) {
          ...
          setAdvancePaid(proformaAdvancePaid);  // ← ADD THIS LINE
          setAdvanceAmount(sourceProforma.advanceAmount || 0);  // ← ADD THIS LINE
          setDueAmount(proformaDueAmount);
  ...
  useEffect(() => {
    const loadQuotationForConversion = async () => {
      //    console.log("=== DEBUG: loadQuotationForConversion called ===");
      // console.log("sourceQuotation:", sourceQuotation);
      // console.log("isFromQuotation:", isFromQuotation);
      // console.log("sourceQuotation.customerId:", sourceQuotation?.customerId);
      // console.log("sourceQuotation.customerId type:", typeof sourceQuotation?.customerId);
      if (sourceQuotation && isFromQuotation) {
        ...
        if (sourceQuotation.customerId) {
          //     console.log("=== Processing quotation conversion ===");
          // console.log("Full sourceQuotation:", JSON.stringify(sourceQuotation, null, 2));
        ...
        toast.info("Quotation loaded. Review and confirm to create Sales Order.");
      }
    };
  ...
  //     return `${prefix}${year}${month}${sequence}`;
  //   };
  //   setInvoiceNo(generateOrderNumber()); // Keep using invoiceNo state but with ORD prefix
  // }, []);
  ...
  useEffect(() => {
    ...
    setDueAmount(grandTotal - receivedAmount);
  ...
  //       notes: "",
  //       termsAndConditions: "",
  //       sourceQuotationId: quotationIdRef || null,
  //       sourceProformaId: proformaIdRef || null,
  ...
  //           toast.error(error?.response?.data?.message || "Could not update quotation status")
  //         }
  //       }

  //       if (shouldPrint) {
  //         navigate(`/skeleton?redirect=/show-sales-order/${response.data.salesOrder._id}`);
  //       } else {
  //         navigate("/skeleton?redirect=/online-orders");
  //       }
  //     }
  //   } catch (error) {
  ...
  //   }
  // };
  // Handle form submission for Sales Order with attachments
  const handleSubmit = async (shouldPrint = false) => {
    if (!customer.customerId) {
      toast.error("Please select a customer first");
      return;
    ...
    try {
      ...
      // Add all sales order data as fields
      formData.append("customerId", customer.customerId);
      formData.append("orderDate", invoiceDate.toISOString());
      formData.append("expectedDeliveryDate", dueDate.toISOString());
      ...
      // Add source IDs if converting
      if (quotationIdRef) {
        formData.append("sourceQuotationId", quotationIdRef);
      }
      if (proformaIdRef) {
        formData.append("sourceProformaId", proformaIdRef);
      }
      ...
      const response = await api.post("/api/sales-orders", formData, {
        headers: {
      ...
      if (response.data.success) {
        ...
        if (isFromQuotation && quotationIdRef) {
          try {
            ...
            console.log("Could not update quotation status:", error.message);
          }
        }
        ...
        <div className="">
          ...
          >
            ...
            >
              ...
              >
                {id ? (isReadOnly ? "View Sales Order" : "Edit Sales Order") :
                  (isConvertingFromProforma ? "Convert Proforma to Sales Order" :
                    (isConvertingFromQuotation ? "Convert Quotation to Sales Order" :
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            <input
                              ...
                              style={{
                                ...
                                backgroundColor: isReadOnly ? "#f5f5f5" : "transparent",  // Add this
                                color: isReadOnly ? "#6c757d" : "inherit",  // Add this
                              }}
                              readOnly={isReadOnly}
                              value={
                          ...
                          {isFromNavbar && showCustomerDropdown && !isReadOnly && (
                            <div
                              style={{
                                position: "absolute",
                                top: "100%",
                      ...
                      <div>
                        <textarea
                          ...
                          style={{
                            ...
                            marginTop: "4px",
                            resize: "none",
                            backgroundColor: isReadOnly ? "#f5f5f5" : "white",  // Add this
                            color: isReadOnly ? "#6c757d" : "inherit",  // Add this
                          }}
                          readOnly={isReadOnly}
                      ...
                      >
                        ...
                        <div style={{ position: "relative", width: 200 }}>
                          ...
                          <div
                            ...
                            style={{
                              ...
                              alignItems: "center",
                              justifyContent: "space-between",
                              cursor: isReadOnly ? "default" : "pointer",  // Change cursor
                              background: isReadOnly ? "#f5f5f5" : "#fff",  // Add background
                            }}
                            onClick={() => {
                              if (isReadOnly) return;  // Prevent click in view mode
                              e.stopPropagation();
                        ...
                        <div style={{ position: "relative", width: 200 }}>
                          ...
                          <div
                            ...
                            onClick={() => { if (isReadOnly) return; closeAllDropdowns(); setViewDueDateOptions(true) }}
                          >
                            <LuCalendarMinus2 />
                            <div
                              style={{
                          ...
                          >
                            ...
                            <input
                              ...
                              style={{
                                ...
                                fontFamily: "Inter",
                                fontWeight: "400",
                                background: isReadOnly ? "#f5f5f5" : "transparent",  // Add this
                                color: isReadOnly ? "#6c757d" : "inherit",
                              }}
                              readOnly={isReadOnly || !isEditMode} // Only editable in edit mode, or remove this line to always allow editing
                            />
                          </div>
                        </div>
                      </div>
                            ...
                            >
                              <input
                                ...
                                style={{
                                  ...
                                  backgroundColor: isReadOnly ? "#f5f5f5" : "white",  // Add this
                                  color: isReadOnly ? "#6c757d" : "inherit",  // Add this
                                  cursor: isReadOnly ? "default" : "text",  // Add this
                                }}
                              />
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {isReadOnly ? "Order Information" : "Payment Details"}
                  </div>
                  ...
                  >
                    <div>
                      ...
                      >
                        <input
                          ...
                          disabled={isReadOnly}
                        />
                        <span style={{ opacity: isReadOnly ? 0.6 : 1 }}>Shopping Points</span>
                      </div>

                      <div style={{ fontSize: "12px", margin: "8px 0", opacity: isReadOnly ? 0.6 : 1 }}>
                    ...
                    <div>
                      ...
                      >
                        ...
                        >
                          <span style={{ fontSize: "14px", color: "#6b7280", opacity: isReadOnly ? 0.6 : 1 }}>
                            Point Used
                          </span>
                          <div
                            style={{
                          ...
                          >
                            <input
                              ...
                              style={{
                                width: "30px",
                                border: "none",
                                background: isReadOnly ? "#f5f5f5" : "transparent",
                                textAlign: "center",
                                fontSize: "14px",
                                outline: "none",
                                backgroundColor: "white",
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div
                        style={{
                          ...
                          padding: "10px",
                          border: "1px solid #e5e7eb",
                          background: isReadOnly ? "#f5f5f5" : "#f9fafb",
                          outline: "none",
                          display: "flex",
                        }}
                      >
...
export default CustomerCreateSalesOrder;
