...
function ShowDeliveryChallan() {
  ...
  useEffect(() => {
    const fetchChallan = async () => {
      try {
        ...
        if (res.data.success) {
          setChallanData(res.data.deliveryChallan);
        } else {
          toast.error("Failed to load delivery challan");
          navigate("/delivery-challans");
        }
      } catch (err) {
        toast.error(err?.response?.data?.message || "Failed to load delivery challan");
        navigate("/delivery-challans");
      } finally {
        setLoading(false);
      }
  ...
  // Auto print if coming from print page
  useEffect(() => {
    if (location.state?.autoPrint && challanData) {
      setTimeout(() => {
        window.print();
      }, 500);
    }
  }, [location.state, challanData]);
...
const fetchSignature = async () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      pdf.save(`delivery-challan-${challanData?.challanNo || "challan"}.pdf`);
    } catch (error) {
  ...
  const shareChallanSms = async () => {
    try {
      setShareLoadingSms(true);
      toast.info("SMS sharing will be implemented soon");
    } catch (error) {
      toast.error("Failed to share via SMS");
    } finally {
      setShareLoadingSms(false);
    }
  };

  const shareChallanWhatsapp = async () => {
    try {
      setShareLoadingWhatsapp(true);
      toast.info("WhatsApp sharing will be implemented soon");
    } catch (error) {
      toast.error("Failed to share via WhatsApp");
    } finally {
      setShareLoadingWhatsapp(false);
    }
  };

  const shareChallanEmail = async () => {
    try {
      setShareLoadingId(true);
      toast.info("Email sharing will be implemented soon");
    } catch (error) {
      toast.error("Failed to send email");
    } finally {
      setShareLoadingId(false);
    }
  };

  if (loading) return <div className="d-flex justify-content-center align-items-center" style={{ height: "100vh" }}>Loading...</div>;
  if (!challanData) return <div>Delivery Challan not found</div>;

  const customer = challanData.customerId
    ? {
      name: challanData.customerId.name,
      address: (() => {
        const addr = challanData.customerId;
        const parts = [];
      ...
      phone: challanData.customerId.phone,
      email: challanData.customerId.email,
      gstin: challanData.customerId.gstin
    }
    : {};
  ...
  return (
    <div className="p-4" style={{ height: "100vh" }}>
      ...
      <div style={{ height: "calc(100vh - 70px)", overflow: "auto" }}>
        {/* back button */}
        <Link to="/deliverychallan" style={{ textDecoration: "none" }}>
          <span
            style={{
              cursor: "pointer",
              position: "fixed",
        ...
        >
          ...
          >
            ...
            >
              <p style={{ fontSize: 18, fontWeight: 700, margin: 0, padding: 0, color: "#474951", fontFamily: "Inter" }}>
                Delivery Challan
              </p>
            </div>
...
export default ShowDeliveryChallan;