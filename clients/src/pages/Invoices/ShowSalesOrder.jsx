...
import { SalesOrderContent } from "./PreviewSalesOrder.jsx";
  ...
  useEffect(() => {
    const fetchSalesOrder = async () => {
      try {
        ...
        setSalesOrderData(res.data.salesOrder);
      } catch (err) {
...
const handleDownloadPDF = async () => {
  if (!salesOrderRef.current) return;
  ...
  try {
    ...
    const element = salesOrderRef.current;
    ...
    pdf.save(`sales-order-${salesOrderData?.salesOrderNo || "order"}.pdf`);
  } catch (error) {
  ...
  if (loading) return <div>Loading sales order...</div>;
  if (!salesOrderData) return <div>Sales Order not found</div>;

  const customer = salesOrderData.customerId
    ? {
        name: salesOrderData.customerId.name,
        address: (() => {
          const addr = salesOrderData.customerId;
          const parts = [];
        ...
        phone: salesOrderData.customerId.phone,
        email: salesOrderData.customerId.email,
        gstin: salesOrderData.customerId.gstin,
      }
    : {};
  ...
  return (
    <div className="p-4" style={{ height: "100vh" }}>
      <div style={{ height: "calc(100vh - 70px)", overflow: "auto" }}>
        {/* back button */}
        <Link to="/online-orders" style={{ textDecoration: "none" }}>
          <span
            style={{
              cursor: "pointer",
              position: "fixed",
