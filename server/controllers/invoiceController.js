...
exports.printSalesInvoice = async (req, res) => {
    try {
        const { Sales: SalesModel } = await getAutoModels(req);

        const { invoiceId } = req.params;
        const sale = await SalesModel.findOne({ invoiceId })
...
exports.createInvoice = async (req, res) => {
    try {
        const { Invoice: InvoiceModel } = await getAutoModels(req);
...
exports.getAllInvoice = async (req, res) => {
    try {
        const { Sales: SalesModel, Invoice: InvoiceModel, Customer: CustomerModel } = await getAutoModels(req);
        ...
        const salesInvoiceIds = await SalesModel.find({ invoiceId: { $exists: true, $ne: null } }).distinct("invoiceId");
        ...
        if (search) {
            ...
            const matchingSales = await SalesModel.find({
        ...
        const merged = await Promise.all(
            invoices.map(async (inv) => {
                let sale = null;
                try {
                    sale = await SalesModel.findOne({ invoiceId: inv.invoiceId });
                } catch (e) {
                    sale = null;
                }
                // Calculate totals
...
exports.bulkDeleteInvoice = async (req, res) => {
    try {
        ...
        const result = await InvoiceModel.deleteMany({ _id: { $in: ids } });
