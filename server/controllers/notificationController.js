...
const NotificationModelFactory = require("../models/notificationModel");

// helper: tenant notification model
const getNotificationModel = async (req) => {
  // Check both req.tenant and req.user.tenant for compatibility
  const tenantInfo = req.tenant || req.user?.tenant;
  
  if (!tenantInfo || !tenantInfo.dbName) {
    throw new Error("Tenant DB not found in request");
  }

  const conn = await getTenantDB(tenantInfo.dbName);
  return NotificationModelFactory.forTenant(conn);
};
...
exports.createNotification = async (
  ...
  try {
    const conn = await getTenantDB(tenantDbName);
    const NotificationModel = NotificationModelFactory.forTenant(conn);
...
exports.getNotifications = async (req, res) => {
  try {
    const { Notification: NotificationModel } = await getNotificationModel(req);
...
exports.deleteSelectedNotification = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Failed to delete selected notifications", error: error.message });
  }
};
